...
class State:
    def __init__(self, position=['&'], parent=None,n_obj=8):
        ...
        self.child_nodes = []   
 #这段代码是Python中的一个类的初始化方法。该类的实例用于实现蒙特卡罗树搜索算法中的节点。其中，position是节点的位置，visits是节点被访问的次数，total_reward是节点的总奖励值，parent_node是节点的父节点，child_nodes是节点的子节点列表。在初始化时，position默认为['&']，parent默认为None。       
...
#这段代码是Python中的一个类，用于实现蒙特卡罗树搜索算法中的节点。该类有一个初始化方法，用于初始化节点的位置、访问次数、总奖励值、父节点和子节点列表等属性。其中，position是节点的位置，visits是节点被访问的次数，total_reward是节点的总奖励值，parent_node是节点的父节点，child_nodes是节点的子节点列表。在初始化时，position默认为['&']，parent默认为None。

#该类还有一个clone方法，用于复制节点。该方法可以选择是否复制访问次数、总奖励值、父节点和子节点列表等属性。该方法返回一个新的State对象。

#最后，该类还有一个add_position方法，用于向节点的位置列表中添加一个新的位置
...
class Node:
    ...
    def select_node(self, logger):#选出得分最高的节点（从得分最高的节点中随机选择一个节点）#选择节点的算法这部分需要改，要结合pareto
        ...
        score_list = []
        logger.debug('UCB:')
        print('self.state.child_nodes is here')
        print(self.state.child_nodes)
        for i in range(len(self.state.child_nodes)):
            print(self.state.child_nodes[i].state)
        for i in range(len(self.state.child_nodes)):
            score = self.policy_evaluator.evaluate(self.state.child_nodes[i].state, self.conf)#每个子节点的得分
            score_list.append(score)#将得分添加到得分list中
            logger.debug(f"{self.state.child_nodes[i].position} {score}") 
        m = np.amax(score_list)#取最大值
        indices = np.nonzero(score_list == m)[0]#查找最大值的索引
        ind = int(self.conf['random_generator'].choice(indices))#用随机数生成器在相同得分的节点之间随机选择，从而更好地探索路径
    ...
    def simulation(self):#如果程序执行到这个方法，就会直接退出，并且输出错误信息。这段代码的作者可能是想防止其他人误用这个方法，或者是在开发过程中暂时禁用了这个方法。
...
class MCTS:
    def __init__(self, root_state, conf, tokens, model, reward_calculator, policy_evaluator, logger):
        ...
        if conf['batch_reward_calculation']:#根据batch_reward_calculation的值是真是假来获取不同类型的目标函数
        ...
        self.output_path = os.path.join(conf['output_dir'], f"result_C{conf['c_val']}.csv")#生成一个输出文件的路径
        if os.path.exists(self.output_path) and not conf['restart']:
            sys.exit(f"[ERROR] {self.output_path} already exists. Please specify a different file name.")#如果已经存在这个路径则提醒这句话
        ...
        if conf['threshold_type'] == "time":
            self.threshold = time.time() + 3600 * conf['hours']#当前时间(unix纪元从1970年1月1日到今天所经历的秒数+一定秒数
    ...
    def flush(self):#这段代码是Python中的一个类的方法，名为flush，它的作用是将缓冲区中的数据写入到文件中
        #print('here')
        #print(len(self.generated_id_list))
        #print(len(self.valid_smiles_list))
        #print(len(self.reward_values_list))
        #print(len(self.depth_list))
        #print(len(self.elapsed_time_list))
        #print(len(self.filter_check_list))

        df = pd.DataFrame({#它使用pandas库创建一个DataFrame对象，其中包含了一些列数据，即为生成的excel表格里的数据
        ...
        df = pd.concat([df, df_obj], axis=1)#将两个数据框（dataframe）按列方向（axis=1）进行合并（concat）
        if os.path.exists(self.output_path):#判断一个路径是否存在
            df.to_csv(self.output_path, mode='a', index=False, header=False)#名为df的数据框写入到一个csv文件中，mode='a'表示以追加模式写入文件，index=False表示不将行索引写入文件，header=False表示不将列名写入文件。
        else:
            df.to_csv(self.output_path, mode='w', index=False)#mode='w'表示以覆盖模式写入文件，index=False表示不将行索引写入CSV文件中


        self.logger.info(f"save results at {self.output_path}")

        self.generated_id_list.clear()#clear()"是一个列表的方法，用于清空列表中的所有元素。
    ...
    def search(self):#search()方法的作用是执行搜索操作
        """initialization of search tree"""
        slection = 1
        expansion = 1
        back = 1
        ckpt_path = os.path.join(self.conf['output_dir'], self.conf['checkpoint_file'])
        if self.conf['restart'] and os.path.exists(ckpt_path):#如果self.conf['restart']为True且ckpt_path文件存在，则程序会重启，self.conf是一个配置文件，而os.path.exists()是Python中的一个函数，用于检查文件是否存在。
        ...
        while (time.time() if self.conf['threshold_type']=="time" else self.total_valid_num) <= self.threshold:#小于设定的阈值的时候就进行以下步骤
            ...
            for _ in range(self.conf['simulation_num']):
                ...
                for n in nodeadded_tmp:
                    ...
                    all_posible = chem_kn_simulation(self.model, position_tmp, self.tokens, self.conf)#这个函数用模型生成smiles序列
                    #print('all_posible is here')
                    #print(all_posible)
            ...
            print(self.logger)            
            print(_gids)
            node_index, objective_values, valid_smiles, generated_id_list, filter_check_list ,true_values = evaluate_node(new_compound, self.generated_dict, self.reward_calculator, self.conf, self.logger, _gids)#这个函数的目的是计算这些生成的新化合物的目标值
            #在这里得到val_list=[]记录每个分子对每个goal的分数
            #print('node_index is here')
            #print(node_index)
            #print(nodeadded)
            #print(node_index)
            #print(valid_smiles)
                    ...
                    for smiles4 in tmp_back:
                        ...
                        for i in range(n):
                            if values3[i] > values4[i]:
                                score = score + 1
                        #if values3[0] > values4[0]:
                        #    score = score + 1
                        #if values3[1] > values4[1]:
                        #    score = score + 1
                        ##if values3[2] > values4[2]:
                         #   score = score + 1
                        #if values3[3] > values4[3]:
                        ##    score = score + 1    
                        if score == n:
                ...
                for smiles1 in valid_smiles:
                    for smiles2 in self.valid_smiles_list:
                        ...
                        if self.dominate(smiles1,smiles2):
                            ...
                            #for index, smiles in enumerate(self.valid_smiles_list):
                            #    if smiles != smiles2:
                            #        tmp_valid_smiles.append(smiles)
                            #        #tmp_node_index.append(node_index[index])
                            #        tmp_objective_values_list.append(self.objective_values_list[index])
                            #        tmp_generated_id_list.append(self.generated_id_list[index])
                            #        tmp_filter_check_list.append(self.filter_check_list[index])
                            #self.valid_smiles_list = tmp_valid_smiles
                            ##node_index = tmp_node_index
                            #self.objective_values_list = tmp_objective_values_list
                            #self.generated_id_list = tmp_generated_id_list
                            #self.filter_check_list = tmp_filter_check_list
                ...
                for smiles1 in valid_smiles_to_delete:
                    if smiles1 in valid_smiles:
                        ...
                        true_values.pop(index1)
                            ...
                            #for index, smiles in enumerate(valid_smiles):
                            #    print('index')
                            #    print(index)
                            #    if smiles != smiles1:
                            #        tmp_valid_smiles.append(smiles)
                            #        tmp_node_index.append(node_index[index])
                            #        tmp_objective_values_list.append(objective_values[index])
                            #        tmp_generated_id_list.append(generated_id_list[index])
                            #        tmp_filter_check_list.append(filter_check_list[index])
                            #valid_smiles = tmp_valid_smiles
                            #node_index = tmp_node_index
                            #objective_values = tmp_objective_values_list
                            #generated_id_list = tmp_generated_id_list
                            #filter_check_list = tmp_filter_check_list
    ...
    def load_checkpoint(self):#加载ckpt文件
        ckpt_path = os.path.join(self.conf['output_dir'], self.conf['checkpoint_file'])#据配置文件中的输出目录和检查点文件名拼接出检查点文件的路径
        with open(ckpt_path, mode='rb') as f:#这段代码使用Python的内置函数open打开一个文件，文件路径为ckpt_path。mode参数为'rb'，表示以二进制读取模式打开文件。打开文件后，使用with语句来自动关闭文件。
            cp_obj = pickle.load(f)#从文件对象f中加载一个pickle对象，并将其赋值给变量cp_obj
        self.gid = cp_obj['gid']#将cp_obj中gid的值赋给self.gid属性
