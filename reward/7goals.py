...
class seven_goals(Reward):
    ...
    def get_objective_functions(conf):
        def EGFR(mol): return seven_goals._predict_scaled(mol, "EGFR", lambda x: max_gauss(x, a=1, mu=9, sigma=3))
        def Solubility(mol): return seven_goals._predict_scaled(mol, "Sol", lambda x: max_gauss(x, a=1, mu=0.3, sigma=1.7))
        def Permeability(mol): return seven_goals._predict_scaled(mol, "Perm", lambda x: max_gauss(x, a=1, mu=1.8, sigma=0.5))
        def Metabolic_stability(mol): return seven_goals._predict_scaled(mol, "Meta", lambda x: max_gauss(x, a=1, mu=110, sigma=32))
        def QED(mol): return seven_goals._safe_qed(mol)
        def Toxicity(mol): return seven_goals._predict_scaled(mol, "Tox", lambda x: min_gauss(x, a=1, mu=1.4, sigma=1))
        def SAScore(mol): return seven_goals._safe_sascore(mol)

        return [EGFR, Solubility, Permeability, Metabolic_stability, QED, Toxicity, SAScore]
